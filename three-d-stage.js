  ...
  class ThreeDStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._boot().catch((err) => {
        ...
        const esGL = /webgl|context/i.test(String(err && err.message || err));
        if (esGL) {
          this._err.style.flexDirection = 'column';
          this._err.textContent = 'No se pudo crear el contexto WebGL (límite de contextos o GPU saturada, normal tras muchas recargas seguidas).';
          const b = document.createElement('button');
          b.textContent = 'Reintentar';
          b.style.cssText = 'margin-top:12px;padding:9px 22px;font:600 13px system-ui;border-radius:5px;border:1px solid #999;background:#fff;cursor:pointer';
          b.addEventListener('click', () => location.reload());
          this._err.appendChild(b);
        } else {
    ...
    async _boot() {
      ...
      // La creacion del contexto WebGL puede fallar transitoriamente (limite
      // de contextos del navegador tras varias recargas, GPU saturada): se
      // reintenta unas veces antes de rendirse — los contextos viejos se
      // liberan en cuanto el recolector pasa.
      let renderer = null, glErr = null;
      for (let intento = 0; intento < 5 && !renderer; intento++) {
        if (intento) await new Promise((r) => setTimeout(r, 650));
        try {
      ...
      if (!renderer) throw new Error('WebGL: ' + (glErr && glErr.message ? glErr.message : 'no context'));
      // OJO: nada de forceContextLoss() en pagehide — Chrome lo interpreta como
      // "la pagina provoco la perdida de contexto" y BLOQUEA la creacion de
      // contextos en las recargas siguientes. El navegador ya libera los
      // contextos al navegar; con el bucle de reintentos basta.
